...
import { fetchBookmark, updateBookmark, addTagsToBookmark } from './actions.js'
...
const DEFAULT = Immutable.fromJS({
  list: {},
  isPending: false,
  isPendingTags: false, // When updating the bookmark tags
  lastUpdated: null,
  error: null,
})
 ...
 */
const onAddTagsRequest = (state, action) =>
  state.merge({
    isPendingTags: true,
...
const onAddTagsSuccess = (state, action) =>
  state.merge({
    isPendingTags: false,
    list: {
      ...state.get('list').toJS(),
...
// TODO
const onAddTagsFailure = (state, action) =>
  state.merge({
    isPendingTags: false,
  })
